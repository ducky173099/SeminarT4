...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        InvokeRepeating("CalculatePath", 0f, repeatTimeUpdatePath); //invokeRepeating để gọi một hàm lặp lại theo thời gian xác định
    }

    void CalculatePath()
    {
        if (seeker.IsDone())
            seeker.StartPath(rb.position, target.position, OnPathCompleted);//ham bat dau tinh toan tiep can muc tieu
    }
    ...
    void MoveToTarget() //di chuyen den muc tieu
    {
    ...
    void DamagePlayer()
    {
        ...
        //
        PlayerHealth.GetComponent<PlayerController>().TakeDamageEffect(damage);
    }
